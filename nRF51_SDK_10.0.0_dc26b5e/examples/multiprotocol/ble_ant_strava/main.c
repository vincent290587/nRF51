...
void app_error_handler(uint32_t error_code, uint32_t line_num, const uint8_t *p_file_name) {
  ...
  if (p_file_name) {
    printf("$DBG,1,%lu,%lu,%s\n\r", (unsigned long int)error_code, (unsigned long int)line_num, p_file_name);
  } else {
    printf("$DBG,0,%lu,%lu\n\r", (unsigned long int)error_code, (unsigned long int)line_num);
  }
        ...
        case ANT_HRM_PAGE_4_UPDATED:
            ...
            if ((beat_count - s_previous_beat_count) == 1)
            {
                ...
                ble_hrs_rr_interval_add(&m_hrs, beat_time - prev_beat);
							  ...
							  rrInterval_ms = (float)(beat_time - prev_beat) * 1000. / 1024.;
...
void bsp_evt_handler(bsp_event_t evt)
{
    ...
    switch (evt)
    {
        ...
        case BSP_EVENT_KEY_3:
				  ...
				  printf("$BTN,%u\n\r", (unsigned int)(evt - BSP_EVENT_KEY_0));
